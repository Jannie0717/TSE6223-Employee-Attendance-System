...
if ($status === 'Not Late') { $sql .= " AND ar.lateStatus = 'Not Late' AND ar.attendanceStatus <> 'Absent'"; }
if ($status === 'Late') { $sql .= " AND ar.lateStatus = 'Late' AND ar.attendanceStatus <> 'Absent'"; }
if ($status === 'Absent') { $sql .= " AND ar.attendanceStatus = 'Absent'"; }
...
<p class="page-subtitle">Employee attendance history by recording the date, day, clock-in time, clock-out time and status.</p>
...
<div class="grid grid-3">
    <div class="card stat-card"><div class="stat-number"><?= e($totalFound) ?></div><div class="stat-label">Total Records Found</div></div>
    <div class="card stat-card"><div class="stat-number"><?= e($lateToday) ?></div><div class="stat-label">Number of Late Employees</div><a class="btn btn-blue" href="late.php">▶ View</a></div>
    <div class="card stat-card"><div class="stat-number"><?= e($presentToday) ?></div><div class="stat-label">Number of Employees Present Today</div></div>
</div>

<div class="top-actions" style="margin-top:16px">
    <a class="btn btn-outline" href="records.php?<?= e(http_build_query(array_merge($_GET,['export'=>'csv']))) ?>">Export Excel</a>
...
<div class="table-wrap print-area">
    ...
    <table>
        <thead><tr><th>Employee ID</th><th>Full Name</th><th>Department</th><th>Date</th><th>Clock-In Time</th><th>Clock-Out Time</th><th>Status</th><th class="no-print">Action (Edit)</th></tr></thead>
        <tbody>
        ...
        <?php else: while($r=$records->fetch_assoc()): ?>
            <tr>
                ...
                <td class="table-actions no-print"><a href="update_attendance.php?id=<?= e($r['recordID']) ?>">Edit</a></td>
            </tr>
        <?php endwhile; endif; ?>
        </tbody>
    </table>
